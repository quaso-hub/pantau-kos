"""
bot/formatter.py  v3.2  God Eye Intelligence Brief
MarkdownV2 format, intelligence-brief tone, no emoji in text.
"""
...
def _fmt_price(val: Optional[int], raw_prices: list) -> str:
    if val:
        formatted = f"Rp{val:,}".replace(",", ".")
...
def _financials_section(result: AnalysisResult) -> str:
    ...
    if hasattr(result, "price_score") and result.price_score is not None:
...
def _logistics_section(result: AnalysisResult) -> str:
    ...
    nearby_found = [i for i in (maps.nearby or []) if i.get("found")]
    if nearby_found:
        poi_parts = []
        for poi in nearby_found[:4]:
            t = escape_md(poi.get("type", "?"))
            n = escape_md(poi.get("name", "-"))
            poi_parts.append(f"{t}:{n}")
        lines.append(f"\\- `nearby: {', '.join(poi_parts)}`")
    if maps.air_quality and isinstance(maps.air_quality, dict):
        aqi = escape_md(str(maps.air_quality.get("aqi", "?")))
        cat = escape_md(maps.air_quality.get("category", "?"))
    ...
    if result.deepseek_raw:
        for line in result.deepseek_raw.splitlines():
            ...
            if stripped and (
                ...
                or any(stripped.lower().startswith(kw) for kw in ("risiko", "flag", "peringatan", "warning"))
            ):
...
def _warning_section(warnings: list) -> str:
    if not warnings:
        return ""
    lines = ["*\\[WARNING\\]*"]
    for w in warnings:
        lines.append(f"`code: {escape_md(w)}`")
    return "\n".join(lines)
...
def format_report(result: AnalysisResult) -> str:
    ...
    if warnings:
        ...
        sections.append(_warning_section(warnings))
    return "\n".join(sections)
